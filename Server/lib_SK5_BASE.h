...
unsigned long WINAPI Fonction1(void* params)
{
	...
	char buf[512]; // If changed, line 22 in main.cpp is a change too
	...
	while (TRUE)  // s_Client <-> s_SK5
	{
		if ((result = select(nfds, &readfds, 0, 0, &tv)) > 0)
		{
			if (FD_ISSET(s_CLIENT, &readfds)) // s_Client -> s_SK5
			{
				...
				if (recvd_A <= 0)
				{
					printf("[!] s_Client -> s_SK5 recvd (%i) <= 0\n", recvd_A);
			...
			if (FD_ISSET(s_SK5, &readfds))  // s_SK5-> s_Client
			{
				...
				if (recvd_B <= 0)
				{
					printf("[!] s_SK5-> s_Client recvd (%i) <= 0\n", recvd_B);
					closesocket(s_CLIENT);
					return 4;
				}
...
static int create_sock_thread(void* f)
{
	HANDLE handle;

	if (!(handle = CreateThread(NULL, 0, Fonction1, f, 0, 0)))
		return 0;
